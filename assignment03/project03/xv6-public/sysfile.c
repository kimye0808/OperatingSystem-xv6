...
create(char *path, short type, short major, short minor)
{
  ...
  if((ip = ialloc(dp->dev, type)) == 0)// allocate inode and set type
    panic("create: ialloc");
...
sys_symLink(void)
{
  ...
  begin_op();
  if ((ip = create(new, T_SYMLINK, 0, 0)) == 0) { // create new file and get inode
    end_op();
    return -1;
  }
  int old_len = strlen(old);
  writei(ip, (char*)&old_len, 0, sizeof(int)); // write to inode for reading old path
  ...
  if(omode & O_CREATE){
    ...
    if(ip->type == T_SYMLINK)
    {
      ...
      if((ip = namei(wanted_path)) == 0){//get inode of path
        cprintf("namei(path) symbolic error\n");
        end_op();
        return -1;
      }
